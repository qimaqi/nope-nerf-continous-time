...
from model.posenet import TransNet, RotsNet
from model import rotation_conversions
...
class LearnPoseNet(nn.Module): # _quad 4 
    def __init__(self, num_cams, learn_R, learn_t, cfg, init_c2w=None):
        """
        :param num_cams:
        :param learn_R:  True/False
        ...
        :param init_c2w: (N, 4, 4) torch tensor
        """
        super(LearnPoseNet, self).__init__()
        self.num_cams = num_cams
        self.init_c2w = None
        if init_c2w is not None:
            self.init_c2w = nn.Parameter(init_c2w, requires_grad=False)
    ...
    def forward(self, cam_id):
        ...
        if self.record[cam_id] == 0 and cam_id !=self.num_cams - 1: #do not record yet in this iteration
