...
class TransNet(torch.nn.Module):
    ...
    def __init__(self,cfg):
        super().__init__()
        self.cfg = cfg
        self.input_t_dim = cfg['pose']['posenc']*2 + 1
    ...
    def forward(self, index):
        ...
        points_enc = self.positional_encoding(index, L= self.cfg['pose']['posenc'])
...
class RotsNet(torch.nn.Module): # quad 4 
    ...
    def forward(self, index):
        ...
        for li,layer in enumerate(self.mlp_quad):
            ...
            if li==len(self.mlp_quad)-1:
                feat[:,1:] = torch_F.tanh(feat[:,1:])
                feat[:,0] =  1*(1 - torch_F.tanh(feat[:,0]))
            else:
                feat = torch_F.relu(feat) # softplus_act(translation_feat)

        norm_rots = torch.norm(feat,dim=-1)
        rotation_feat_norm = feat / (norm_rots[...,None] +1e-16)
