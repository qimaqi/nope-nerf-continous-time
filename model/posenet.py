...
class TransNet(torch.nn.Module):
    ...
    def define_network(self,cfg):
        ...
        L = util.get_layer_dims(cfg.pose.layers_feat)
    ...
    def forward(self, index):
        ...
        points_enc = self.positional_encoding(self.cfg,index, L= 5)
    ...
    def positional_encoding(self,input,L): # [B,...,N]
        shape = input.shape
        freq = 2**torch.arange(L,dtype=torch.float32,device=self.cfg.device)*np.pi # [L]
