...
def eval(cfg):
    ...
    if use_learnt_poses:
        if cfg['pose']['init_pose']:
            ...
            init_pose = None
        learned_pose_param_net = mdl.LearnPoseNet(N_imgs, cfg['pose']['learn_R'], 
                                cfg['pose']['learn_t'], cfg=cfg, init_c2w=init_pose).to(device=device)
        print("pose_check",torch.stack([learned_pose_param_net(i) for i in range(train_dataset['img'].N_imgs_train)]))

        checkpoint_io_pose_rots = mdl.CheckpointIO(out_dir, model=learned_pose_param_net.rotsnet)
        checkpoint_io_pose_trans = mdl.CheckpointIO(out_dir, model=learned_pose_param_net.transnet)
        ...
        print("pose_check after load",torch.stack([learned_pose_param_net(i) for i in range(train_dataset['img'].N_imgs_train)]))
